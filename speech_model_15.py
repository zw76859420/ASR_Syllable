...
'''
   此脚本是用来测试6层dcnn模型，模型的结构参数为32-32-64-64-128-128-512-1422；
'''
...
from readdata_15 import DataSpeech
...
class ModelSpeech():
    ...
    def creat_model(self):
        ...
        conv1 = Conv2D(filters=32 , kernel_size=[3 , 3] , padding='same' , use_bias=True , kernel_initializer='he_normal' , activation='relu' , kernel_regularizer=regularizers.l2(1e-3))(input_data)
        conv1 = BatchNormalization(epsilon=0.0002)(conv1)

        conv2 = Conv2D(filters=32 , kernel_size=[3 , 3] , padding='same' , use_bias=True , kernel_initializer='he_normal' , activation='relu' , kernel_regularizer=regularizers.l2(1e-3))(conv1)
        conv2 = BatchNormalization(epsilon=0.0002)(conv2)
        maxpool1 = MaxPooling2D(pool_size=[2 , 2] , strides=[2 , 2] , padding='valid')(conv2)
        maxpool1 = Dropout(rate=0.2)(maxpool1)

        conv3 = Conv2D(filters=64 , kernel_size=[3 , 3] , padding='same' , use_bias=True , kernel_initializer='he_normal' , activation='relu' , kernel_regularizer=regularizers.l2(1e-3))(maxpool1)
        conv3 = BatchNormalization(epsilon=0.0002)(conv3)
        conv4 = Conv2D(filters=64 , kernel_size=[3 , 3] , padding='same' , use_bias=True , kernel_initializer='he_normal' , activation='relu' , kernel_regularizer=regularizers.l2(1e-3))(conv3)
        conv4 = BatchNormalization(epsilon=0.0002)(conv4)
        maxpool2 = MaxPooling2D(pool_size=[2 , 2] , strides=[2 , 2] , padding='valid')(conv4)
        maxpool2 = Dropout(rate=0.2)(maxpool2)

        conv5 = Conv2D(filters=128 , kernel_size=[3 , 3] , padding='same' , use_bias=True , kernel_initializer='he_normal' , activation='relu' , kernel_regularizer=regularizers.l2(1e-3))(maxpool2)
        conv5 = BatchNormalization(epsilon=0.0002)(conv5)
        conv6 = Conv2D(filters=128 , kernel_size=[3 , 3] , padding='same' , use_bias=True , kernel_initializer='he_normal' , activation='relu' , kernel_regularizer=regularizers.l2(1e-3))(conv5)
        conv6 = BatchNormalization(epsilon=0.0002)(conv6)
        maxpool3 = MaxPooling2D(pool_size=[2 , 2] , strides=[2 , 2] , padding='valid')(conv6)
        maxpool3 = Dropout(rate=0.2)(maxpool3)

        reshape = Reshape([200 , 3200])(maxpool3)
        ...
        adam = Adam(lr=0.005 , epsilon=1e-6)
